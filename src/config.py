...
@dataclass
class PruningConfig:
    """Конфиг отдельного этапа prune (будущий шаг удаления весов)."""

    target_layer: int | None = None
    strategy: str = "not_implemented"
...
def load_project_config(config_path: str | None = None, stage: str | None = None) -> ProjectConfig:
    ...
    Args:
        ...
        stage: Явно заданный stage (collect/profile/cluster/expert_choice/prune).
    ...
    # Сливаем параметры: base (model + pipeline) + stage-specific (collection/pipeline/cluster/expert_choice/pruning)
    ...
    pruning_raw = stage_raw.get("pruning") or {}
    pruning = PruningConfig()
    pruning.target_layer = pruning_raw.get("target_layer", pruning.target_layer)
    pruning.strategy = pruning_raw.get("strategy", pruning.strategy)
